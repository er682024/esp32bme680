...
static bool check_auth(httpd_req_t *req) {
    ...
    char expected[128];
    snprintf(expected, sizeof(expected), "%s:%s", ADMIN_USER, current_pass);

    if (strcmp((char *)decoded, expected) != 0) {
        ESP_LOGW(TAG, "Auth fallita per utente '%s'", ADMIN_USER);
        httpd_resp_set_status(req, "401 Unauthorized");
        httpd_resp_set_hdr(req, "WWW-Authenticate", "Basic realm=\"ESP32\"");
...
".nav{display:flex;gap:0.8rem;margin-top:1.8rem;flex-wrap:wrap;justify-content:center}"
".nav a{font-size:0.78rem;padding:7px 16px;border-radius:8px;text-decoration:none;"
"color:#8b949e;border:1px solid #30363d;transition:color .2s,border-color .2s}"
...
"<div class='nav'><a href='/'>&#x2190; Monitor</a> &nbsp;|&nbsp; <a href='/update'>OTA Update</a></div>"
...
void http_server_start(void) {
    ...
    config.max_uri_handlers = 11;  // aggiornato per i nuovi endpoint
    ...
    ESP_LOGI(TAG, "Server HTTP avviato — / (monitor)  /wifi  /data  /update  /settings");
}
